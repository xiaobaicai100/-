    ...
    for i=2:length(x)-1
     ...
     for j=2:length(y)-1%%%第n+1/2层的边界条件
         for i=1:length(x)-1:length(x)
            U2(i,j)=1/2*(U(i,j)+U3(i,j))+1/2*delt*(U(i,j+1)-2*U(i,j)+U(i,j-1)-U2(i,j+1)+2*U2(i,j)-U2(i,j-1));
         end
     end
      
        A=ones(1,3);
        A(1)= -1/2*delt;       %存储a
        A(2)= delt+tal*tal;  %存储b
        A(3)=-1/2*delt;      %存储c
        d=zeros(1,length(x)-2);
        for j=2:1:(length(y)-1)
             v=[U2(1,j),U2(length(x),j)]';
             for i=1:length(d)
                 d(i)=tal*tal*U(i+1,j)+delt*(U(i+1,j+1)-2*U(i+1,j)+U(i+1,j-1));
             end
              [U2(:,j)]=TA(A,v,d); %%v，d在不同步骤中不同，需要分别求
        end
     
        A(1)= -delt;       %存储a
        A(2)= 2*delt+tal*tal;  %存储b
        A(3)=-delt;      %存储c
        ...
        for i=2:1:(length(x)-1)
             ...
             for j=1:length(d)
                 d1(j)=tal*tal*U2(i,j+1)+1/2*delt*(U2(i+1,j+1)-2*U2(i,j+1)+U2(i-1,j+1));
             end
