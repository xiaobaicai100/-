...
for i=2:1:length(t)%%开始求每个时间所对应的值
    ...
    for j=1:length(d)
       d(j)=-1/4*V(i-1,j+2)+(1+k)*V(i-1,j+1)+1/4*V(i-1,j)+k/2*(-t(i)*sin(pi*(x(j+1)-t(i)))-t(i-1)*sin(pi*(x(j+1)-t(i-1)))); 
    end
...
for i=2:1:length(t)%%开始求每个时间所对应的值
    %v=[(1+t(i))*sin(pi*(-1-t(i))),(1+t(i))*sin(pi*(1-t(i)))]';
    for j=1:length(d1)
       d1(j)=-1/4*M(i-1,j+2)+(1+k)*M(i-1,j+1)+1/4*M(i-1,j)+k/2*(-t(i)*sin(pi*(x(j+1)-t(i)))-t(i-1)*sin(pi*(x(j+1)-t(i-1)))); 
    end
    M(i,:)=PTS(A,d1);
end
